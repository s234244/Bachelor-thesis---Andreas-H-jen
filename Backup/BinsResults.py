...
from SmartStart2StepV2 import run_two_step
...
def plot_layout(layout_xy, boundary, x_ref, y_ref, wt, title, spacing_D=4):
    ...
    #Uncomment if you want spacing circles
...
def plot_aep_history(aep_history, wake_model_name):
    """
    Plot AEP development over time for 2-step optimization.

    Parameters
    ----------
    aep_history : list of dicts or pd.DataFrame
...
def main():
    ...
    for wake_model_name in wake_models:
        ...
        aep_smart = calc_aep(wf_model, smart_layout, with_wake_loss=True, wd=wd_bins, ws=ws_bins)
        ...
        aep_stage1 = calc_aep(wf_model, stage1_layout, with_wake_loss=True, wd=wd_bins, ws=ws_bins)
        aep_stage2 = calc_aep(wf_model, stage2_layout, with_wake_loss=True, wd=wd_bins, ws=ws_bins)

        print(f"SmartStart only AEP: {aep_smart:.3f} GWh")
        #print(f"Two-step Stage 1 AEP: {aep_stage1:.3f} GWh")
