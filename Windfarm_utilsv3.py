...
def get_site(site_name='HornsRev1', wt=None, boundary_pad=400, x_points=20, y_points=20):
    ...
    xmin = x_ref.min() - boundary_pad
    xmax = x_ref.max() + boundary_pad
    ymin = y_ref.min() - boundary_pad
    ymax = y_ref.max() + boundary_pad
    ...
    x_grid = np.linspace(xmin, xmax, x_points)
    y_grid = np.linspace(ymin, ymax, y_points)

    XX, YY = np.meshgrid(x_grid, y_grid)
    candidate_points = np.column_stack([XX.ravel(), YY.ravel()])

    return boundary, x_ref, y_ref, site, n_wt, x_points, y_points, XX, YY, candidate_points
...
def calc_aep(wf_model, layout_xy, with_wake_loss=False, wd=None, ws=None):
    ...
    return float(
        np.asarray(
            sim_res.aep(with_wake_loss=with_wake_loss).sum().data
        )
    )
